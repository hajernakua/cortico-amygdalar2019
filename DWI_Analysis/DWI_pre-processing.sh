...
#This script was written by Natalie Forde from the TIGR Lab 
...
# if [[ ! -f "${outfile}" ]] ; then
  ...
  cp /fmap/$subj/dwi_fieldmap_Hz.nii.gz $tmpdir/FM.nii.gz
  cp /data/$subj/ses-01/fmap/$subj\_ses-01_magnitude1.nii.gz $tmpdir/Mag.nii.gz
  # cp /data/$subj/ses-01/fmap/$subj\_ses-01_run-01_magnitude1.nii.gz $tmpdir/Mag.nii.gz
  ...
  if [ -f "single.nii.gz" ]; then
    ...
    printf "0 -1 0 0.022200333" > acqparams.txt
  else echo $subj
	fi
